...
csv_file1=$1  # Get the first CSV file from the command line argument
csv_file2=$2  # Get the second CSV file from the command line argument
csv_file3=$3  # Get the third CSV file from the command line argument
csv_file4=$4  # Get the fourth CSV file from the command line argument
n=$5          # Get the upper limit from the command line argument
num_executions=$6  # Number of Verus executions per generated file
...
plot "$csv_file1" using 1:$(($num_executions + 1)):($(($num_executions + 2)) * 1) with errorbars title 'Prime Average' lc rgb "blue" pointtype 3, \
     "$csv_file2" using 1:$(($num_executions + 1)):($(($num_executions + 2)) * 1) with errorbars title 'Triangle Average' lc rgb "red" pointtype 3, \
     "$csv_file3" using 1:$(($num_executions + 1)):($(($num_executions + 2)) * 1) with errorbars title 'Prime(DT) Average' lc rgb "green" pointtype 3, \
     "$csv_file4" using 1:$(($num_executions + 1)):($(($num_executions + 2)) * 1) with errorbars title 'Prime(Overflow) Average' lc rgb "orange" pointtype 3
EOF
